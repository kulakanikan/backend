...
import { generateDashboardAiSummary } from "../services/gemini";
...
// GET /dashboard/ai-summary — ringkasan AI berbasis metrik dashboard
dashboardRoute.get("/ai-summary", async (c) => {
  const { id: userId } = c.get("user");
  ...
  try {
    ...
    const topBatch = (topBatchResult as any).map((row: any) => ({
      jenis_ikan: row.jenis_ikan,
      laba_riil: Number(row.laba_riil || 0),
    }));

    const insights = (insightResult as any).map((row: any) => ({
      jenis_ikan: row.jenis_ikan,
      kondisi_kualitas: row.kondisi_kualitas || "segar",
      rasio_susut_persen: `${(Number(row.rasio_susut_rata || 0) * 100).toFixed(1)}%`,
    }));

    // Generate Gemini Summary
    const summaryText = await generateDashboardAiSummary({
      omzet,
      laba_riil_total: labaRiil,
      stok_tersisa_kg: stokTersisaKg,
      total_piutang: totalPiutang,
    ...
    return c.json(success({ summary: summaryText }));
  } catch (err: any) {
    console.error("Failed to generate AI summary:", err);
    return c.json(success({ summary: "Gagal memuat ringkasan bisnis AI saat ini." }));
  }
});
